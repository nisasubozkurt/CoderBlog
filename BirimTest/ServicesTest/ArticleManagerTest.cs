...
namespace BirimTest.ServicesTest
{
    ...
    public class ArticleManagerTests
    {
        ...
        public async Task GetAsync_NonExistingArticleId_ReturnsError()
        {
            ...
            // Act
            var result = await _articleManager.GetAsync(0); // Assuming 999 is a non-existent ID.
        ...
        public async Task GetAllAsync_WithArticles_ReturnsSuccessWithData()
        {
            ...
            _mockUnitOfWork.Setup(u => u.Articles.GetAllAsync(null,
                It.IsAny<Expression<Func<Article, object>>[]>())).ReturnsAsync(new List<Article> { new Article(), new Article() });
        ...
        // Tests for other methods like GetArticleUpdateDtoAsync, GetAllByNonDeletedAsync, IncreaseViewCountAsync and so on can be similarly structured.
    }
}
